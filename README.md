...
|   2.3   | 18/06 | Javier Tello             | Implementación del bounded context Medical (backend Spring Boot: domain, application, infrastructure e interfaces) e integración del frontend Angular Appointment Management consumiendo `/api/v1/appointments`.|
...
|   3.1   | 08/07 | Javier Tello             | Corrección y actualización del módulo Appointment Management en la Web Application: integración con JWT real, eliminación de ingreso manual de `patientId` y `doctorId`, visualización de citas próximas y pasadas, creación, modificación y eliminación de citas consumiendo `/api/v1/appointments`. Además, corrección del Cap. IV con el Design-Level Event Storming (4.6.1), Class Diagrams por bounded context para el punto 4.7 y actualización del Student Outcome correspondiente a la entrega final. |
